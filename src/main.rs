...
use smithay_client_toolkit::shell::wlr_layer::{
    self, KeyboardInteractivity, LayerShell, LayerShellHandler,
};
use smithay_client_toolkit::{
    activation::{ActivationHandler, ActivationState},
    compositor::{CompositorHandler, CompositorState},
    delegate_activation, delegate_compositor, delegate_output, delegate_pointer, delegate_registry,
...
use smithay_client_toolkit::{delegate_keyboard, delegate_layer};
...
fn main() {
    ...
    let surface = if let Ok(layer_shell) = LayerShell::bind(&globals, &qh) {
        ...
        layer.set_margin(100, 0, 0, 100); // TODO: set offset here
        ...
        window.set_title("TODO: set name".to_string());
...
impl ActivationHandler for SimpleWindow {
    ...
    fn new_token(&mut self, token: String, _data: &Self::RequestData) {
        self.xdg_activation
            .as_ref()
            .unwrap()
            .activate::<SimpleWindow>(&self.window, token);
    }
}
...
impl SimpleWindow {
    pub fn draw(&mut self, qh: &QueueHandle<Self>) {
        ...
        self.window.frame(qh, self.window.clone());

        // Attach and commit to present.
        buffer.attach_to(&self.window).expect("buffer attach");
        self.window.commit();
    }
}

impl LayerShellHandler for SimpleWindow {
    fn closed(
        &mut self,
        _conn: &Connection,
        _qh: &QueueHandle<Self>,
